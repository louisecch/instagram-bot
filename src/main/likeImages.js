...
export async function patchedLikeCurrentUserImagesPageCode({
  ...
  await globalThis.instautoSleep(2000);

  const allImages = [...document.getElementsByTagName('a')].filter(
    (el) => typeof el.href === 'string' && /instagram.com\/p\//.test(el.href),
  );
  globalThis.instautoLog(`Found ${allImages.length} post link(s) on profile`);
  ...
  globalThis.instautoLog(`Liking ${numImagesToLike} image(s)`);
  const images = imagesShuffled.slice(0, numImagesToLike);

  if (images.length === 0) {
    globalThis.instautoLog('No images to like');
    return;
  }

  for (const image of images) {
    image.click?.();
    await globalThis.instautoSleep(3000);
    ...
    await globalThis.instautoSleep(5000);
  }

  globalThis.instautoLog('Done liking images');
