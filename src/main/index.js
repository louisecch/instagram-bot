...
async function runBotLikePhotosOnly({ usernames, maxLikesPerUser, maxUsersToVisit, skipPrivate } = {}) {
  assert(instauto);

  try {
    logger.log(`Starting like photos only mode for ${usernames.length} accounts, visiting up to ${maxUsersToVisit} users...`);
